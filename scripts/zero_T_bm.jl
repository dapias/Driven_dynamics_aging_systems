...
pf(E)  = rho(E) + (deltat)*(-G(E)*rho(E) + exp(-s)*rho(E)*quadgk(x-> rho(x), 0, E)[1])
t = t0 +deltat
global pf_array = pf.(egrid);

while t < tstops[end]
    integral = [sum(pf_array[1:i].*(egrid[2]  - egrid[1])) for i in 1:length(egrid)]
...
global Deltat = 0.0
q(E)  =  1 + (deltat)*(-G(E)*1 + exp(-s)*quadgk(x-> rho(x), E, Inf)[1])
...
while Deltat < tfinal
    integral = [sum((q_array.*rho_grid*(egrid[2]  - egrid[1]))[i:end]) for i in 1:length(egrid)]
    q1 = deepcopy(q_array)
...
plot!(xx, 0.1*xx.^(-(1)), ls = :dash, color = :blue, lw = 2, label = "")

save("zeroT_s=$(s).jld", "time", tstops[1:end-1], "amean", a_mean[1:end-1])
